 ...
 *
 * This API route handles the OAuth callback from Google/YouTube and exchanges
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(`/?error=${encodeURIComponent(`YouTube authentication failed: ${error}`)}`, request.url)
    );
  }
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(
      new URL(
        `/?error=${encodeURIComponent('YouTube integration not configured')}`,
        request.url
      )
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL(
        `/?youtube_auth=${encodeURIComponent(JSON.stringify(authData))}`,
        request.url
      )
    );
  } catch (err) {
